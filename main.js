...
function createConnection() {
    ...
    if (0) {
        con.rawInput = function(str) { 
            console.log('rawInput');
            addToLog("input", str) 
        };
        con.rawOutput = function(str) { 
            console.log('rawOutput');
            addToLog("output", str) 
        };
    } else {
        con.xmlInput = function(el) { 
            console.log('xmlInput');
            addToLog("input", Strophe.serialize(el)) 
        };
        con.xmlOutput = function(el) { 
            console.log('xmlOutput');
            addToLog("output", Strophe.serialize(el)) 
        };
    }
}
...
try{
    if (!$("#connection_url").val())
        $("#connection_url").val("ws://" + 'bamboojs.com' + ":5280/xmpp");
    enableLogin();
